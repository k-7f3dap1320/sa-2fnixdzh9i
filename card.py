    ...
    try:
        ...
        if t == 9:
            if user_is_login() == 0:
                ...
                "UNION "+\
                "SELECT * FROM (SELECT feed.short_title, feed.short_description, feed.content, feed.url, feed.ranking, feed.badge, feed.symbol FROM feed JOIN instruments ON feed.symbol = instruments.symbol WHERE feed.globalRank <> 0 AND instruments.y1 > 0 AND (feed.asset_class LIKE '%"+x+"%' OR feed.market LIKE '%"+x+"%') AND type=9 ORDER BY feed.globalrank LIMIT 19) AS Q2"
    except:
        if t == 1:
            sql = "SELECT short_title, short_description, content, url, ranking, badge, symbol FROM feed "+\
        ...
        if t == 9:
            if user_is_login() == 0:
                ...
                "ON feed.symbol = instruments.symbol WHERE instruments.owner = "+ str(get_user_numeric_id() ) +" AND feed.type=9 ORDER BY feed.globalRank) AS Q1 "+\
                "UNION "+\
                "SELECT * FROM (SELECT feed.short_title, feed.short_description, feed.content, feed.url, feed.ranking, feed.badge, feed.symbol FROM feed JOIN instruments ON feed.symbol = instruments.symbol WHERE feed.globalRank <> 0 AND instruments.y1 > 0 AND type=9 ORDER BY feed.globalrank LIMIT 19) AS Q2"
    try:
