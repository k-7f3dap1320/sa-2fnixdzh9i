...
def get_card(x,t,burl):
    ...
    if t == 9: return_data = '<div class="box"><span class="sectiont"><i class="fas fa-chart-pie"></i>&nbsp;'+ title_portf +'</span><div class="row">'
    if t == 1: return_data = '<div class="box"><span class="sectiont"><i class="fas fa-chart-line"></i>&nbsp;'+ title_signals +'</span><div class="row">'
    ...
    for row in rs:
        ...
        if t == 1:
            ...
            '<div>&nbsp;</div>'+\
            '                <div class="title"><a href="' + url + '" class="'+badge_class+'" data-toggle="tooltip" data-placement="bottom" title="'+ expl_label +'" >'+badge+'</a>&nbsp;<span class="expl">'+expl_label+'</span></div>'+\
            '            </div>'+\
            '        </div>'
    ...
    if t == 9: return_data = return_data + '</div></div><a href="'+ button_portf_link +'" role="button" class="btn btn-outline-secondary btn-lg btn-block"><strong>'+button_portf+'&nbsp;<i class="fas fa-angle-double-down"></i></strong></a>'
    if t == 1: return_data = return_data + '</div></div><a href="'+ button_signals_link +'" role="button" class="btn btn-outline-secondary btn-lg btn-block"><strong>'+button_signals+'&nbsp;<i class="fas fa-angle-double-down"></i></strong></a>'
