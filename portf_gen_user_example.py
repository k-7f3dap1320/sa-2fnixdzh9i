...
def gen_portf_user_example(burl,acm,notstart):
    ...
    if acm is None:
        ...
        asset_class = acm
    volatility_risk_fx = 0.01
    if asset_class == "FX:":
        volatility_risk = volatility_risk_fx
    else:
    ...
    if user_is_login() == 1:
        ...
        sql = "SELECT symbol_list.uid FROM instruments JOIN symbol_list ON symbol_list.symbol = instruments.symbol WHERE instruments.symbol NOT LIKE '%"+ get_portf_suffix() +"%' AND symbol_list.disabled=0 AND (instruments.y1_signal > 0 AND instruments.volatility_risk_st < "+ str(volatility_risk) +" ) AND (instruments.asset_class LIKE '"+ asset_class +"' OR instruments.market LIKE '"+ asset_class +"') ORDER BY instruments.m6_signal, instruments.m3_signal, instruments.m1_signal LIMIT 5"
        cr.execute(sql)
        rs = cr.fetchall()
        i = 0
        for row in rs:
        ...
        if i < 5:
            ...
            "(instruments.asset_class LIKE '"+ asset_class +"' OR instruments.market LIKE '"+ asset_class +"') AND symbol_list.disabled = 0 ORDER BY instruments.m6_signal, instruments.m3_signal, instruments.m1_signal LIMIT 5"
            cr.execute(sql)
            rs = cr.fetchall()
            i = 0
            for row in rs:
        ...
        if i < 5:
            ...
            "(instruments.asset_class LIKE '"+ add_additional_asset +"') AND symbol_list.disabled = 0 ORDER BY instruments.m6_signal, instruments.m3_signal, instruments.m1_signal LIMIT 5"
            cr.execute(sql)
            rs = cr.fetchall()
            for row in rs:
                resp.set_cookie('portf_s_'+str(i), str(row[0]), expires=datetime.datetime.now() + datetime.timedelta(days=1) )
