...
def gen_portf_user_example(burl,acm):
    ...
    try:
        ...
        if user_is_login() == 1:
            ...
            sql = "SELECT symbol_list.uid FROM instruments JOIN symbol_list ON symbol_list.symbol = instruments.symbol WHERE instruments.symbol NOT LIKE '%"+ get_portf_suffix() +"%' AND (instruments.y1_signal > 0 AND instruments.m1_signal > 0) AND (instruments.asset_class LIKE '"+ asset_class +"' OR instruments.market LIKE '"+ asset_class +"') ORDER BY RAND() LIMIT 5"
            print(sql)
            cr.execute(sql)
            rs = cr.fetchall()
            i = 1
