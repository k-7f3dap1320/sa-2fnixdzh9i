...
def gen_sign_page(uid,appname,burl):
    ...
    try:
        ...
        r = r + get_body( get_loading_body(), navbar(burl) + '<div class="box-top"><div class="row">' + get_details_header(uid,burl) + get_sign_header(uid) + get_sign_ta_chart_alt_orders(uid) + get_sign_recommend_trail_returns(uid) + get_trades_box(uid,burl) + '</div></div>')
        r = set_page(r)
