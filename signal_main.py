...
def gen_sign_page(uid,appname,burl):
    ...
    try:
        ...
        r = get_head(  get_loading_head() + get_googleanalytics() + get_googleadsense() + get_title( appname +' - ' + instfullname ) + get_metatags(burl) + set_ogp(burl,2,page_title,page_desc) + get_bootstrap() + get_awesomplete() + get_tablesorter() + get_font_awesome() + get_google_chart_script() + get_stylesheet(burl) )
        r = r + get_body( get_loading_body(), navbar(burl) + '<div class="box-top"><div class="row">' + get_details_header(uid,burl) + get_sign_header(uid) + get_sign_ta_chart_alt_orders(uid) + get_sign_recommend_trail_returns(uid) + get_trades_box(uid,burl) + '</div></div>' + get_page_footer(burl))
        r = set_page(r)
