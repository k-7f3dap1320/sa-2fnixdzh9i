...
def get_search_result(q):
    ...
    for row in rs:
        ...
        return_data = set_page( get_head('<meta http-equiv="refresh" content="0;URL=' + str(url) + '" />') + get_body('','') )

    if return_data == '':
        return_data = set_page( get_head('<meta http-equiv="refresh" content="0;URL=/?s=0" />') + get_body('','') )
...
def get_search_page(appname,burl,nonavbar):
    ...
    return_data = return_data + get_body( get_loading_body(), navbarcontent + get_search_page_content(burl,nonavbar) + get_page_footer(burl) )
